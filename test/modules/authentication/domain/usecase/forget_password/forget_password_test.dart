...
void main() {
  group('test LoginRepoImpl', () {
    ...
    test('when call forget password function it should call forgetPassword from repo', () async {
      ...
      when(forgetPasswordRepo.forgetPassword(email)).thenAnswer((_) async => result);
      var actual = await forgetPasswordUseCase.call(email,);
      verify(forgetPasswordRepo.forgetPassword(email,)).called(1);
      expect(actual, equals(result));
    });
    test('when call reset password function it should call resetPassword from repo', () async {
      ...
      when(forgetPasswordRepo.resetPassword(email, newPassword)).thenAnswer((_) async => result);
    ...
    test('when call reset code function it should call resetCode from repo', () async {

      var result = Success<ForgetPasswordResponse>(data: ForgetPasswordResponse());

      provideDummy<ApiResult<ForgetPasswordResponse?>>(result);
      var code = '123456';
      when(forgetPasswordRepo.resetCode(code)).thenAnswer((_) async => result);
