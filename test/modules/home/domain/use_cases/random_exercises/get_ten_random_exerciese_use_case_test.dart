...
import 'get_ten_random_exerciese_use_case_test.mocks.dart';

@GenerateMocks([RandomExercisesRepo])
void main() {
  group("GetTenRandomExerciseUseCase class Testing", () {
    ...
    setUpAll(() {
      randomExercisesRepo = MockRandomExercisesRepo();
      getTenRandomExerciseUseCase = GetTenRandomExerciseUseCase(
        randomExercisesRepo,
      );
    });
    test(
      "When calling call() function of the use case, it should call randomExercisesRepo.getRandomExercise() and return success if the latter returns so.",
      () async {
        // arrange
        ...
        when(
          randomExercisesRepo.getRandomExercises(
        ...
        // act
        var useCaseResult = await getTenRandomExerciseUseCase.call();
    ...
    test(
      "When calling call() function of the use case, it should call randomExercisesRepo.getRandomExercise() and return error if the latter returns so.",
      () async {
        // arrange
        provideDummy<ApiResult<RandomExercisesResponseEntity>>(
        ...
        switch (useCaseResult) {
          case Success<RandomExercisesResponseEntity>():
            debugPrint("Impossible Case");
          case Error<RandomExercisesResponseEntity>():
            expect(useCaseResult.error, exception);
        }
      },
    );
  });
