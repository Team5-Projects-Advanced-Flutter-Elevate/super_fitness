...
void main() {
  ...
  setUp(() async {
    ...
    when(
      mockAppLocalizations.priceOfExcellence,
    ).thenReturn('The Price Of Excellence');
    ...
    when(
      mockAppLocalizations.utGravidaQuisIdPretiumPurusMaurisMassa,
      ...
      child: MaterialApp(
        ...
        routes: {
          DefinedRoutes.onboardingScreenRoute:
              (context) => const Scaffold(body: Text('Onboarding Screen')),
        },
      ),
    );
  }
  ...
  group('Storage Components Unit Tests', () {
    group('SecureStorageServiceImp', () {
      ...
      setUp(() {
        secureStorageService = SecureStorageServiceImp(
      ...
      test('setStringValue stores value successfully', () async {
        ...
        verify(
          mockFlutterSecureStorage.write(key: 'testKey', value: 'testValue'),
        ).called(1);
      });
    ...
    group('StorageExecutionHandler', () {
      test('execute returns StorageSuccessResult on success', () async {
        final result = await StorageExecutionHandler.execute<String>(
          () async => 'testValue',
        );
  ...
  group('LocalizationManager Unit Tests', () {
    ...
    test('changeLocal updates locale and stores it', () async {
      ...
      verify(
        mockFlutterSecureStorage.write(
          key: StorageConstants.localeKey,
          value: 'ar',
        ),
      ).called(1);
    ...
    test('getSavedLocal returns stored locale', () async {
      when(
        mockFlutterSecureStorage.read(key: StorageConstants.localeKey),
  ...
  group('OnboardingScreen Widget Tests', () {
    testWidgets('OnboardingScreen renders correctly with first page', (
      WidgetTester tester,
    ) async {
      ...
      expect(
        find.text('Lorem ipsum dolor sit amet consectetur. Eu urna'),
        findsOneWidget,
      );
      expect(
        find.text('ut gravida quis id pretium purus. Mauris massa'),
        findsOneWidget,
      );
    ...
    testWidgets('Tapping Next button navigates to the next page', (
      WidgetTester tester,
    ) async {
      await tester.pumpWidget(createTestableWidget(const OnboardingScreen()));
      await tester.pumpAndSettle();
    ...
    testWidgets('Tapping Back button navigates to the previous page', (
      WidgetTester tester,
    ) async {
      await tester.pumpWidget(createTestableWidget(const OnboardingScreen()));
      await tester.pumpAndSettle();
    ...
    testWidgets('Tapping Do It on the last page navigates to LoginScreen', (
      WidgetTester tester,
    ) async {
      await tester.pumpWidget(createTestableWidget(const OnboardingScreen()));
      await tester.pumpAndSettle();
    ...
    testWidgets('Tapping Skip navigates to LoginScreen', (
      WidgetTester tester,
    ) async {
      await tester.pumpWidget(createTestableWidget(const OnboardingScreen()));
      await tester.pumpAndSettle();
    ...
    testWidgets('OnboardingScreen renders Arabic text after locale change', (
      WidgetTester tester,
    ) async {
      when(mockLocalizationManager.currentLocale).thenReturn('ar');
